...
Before deployment, place every permission-cleared GPX in `data/trails/` and maintain its entry in the adjacent `metadata.json`. The build automatically discovers and ingests every `.gpx`, regenerates the single file-backed catalogue, and packages derived routes into the public demo. Do not add a judge-facing upload step.
