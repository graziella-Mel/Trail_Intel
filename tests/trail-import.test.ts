...
test("canonical trail directory contains every GPX and matching metadata",async()=>{const files=(await readdir("data/trails")).filter(file=>file.toLowerCase().endsWith(".gpx")).sort();const metadata=JSON.parse(await readFile("data/trails/metadata.json","utf8")) as {file:string}[];assert.equal(files.length,17);assert.deepEqual(metadata.map(item=>item.file).sort(),files);assert.equal((await readdir("public")).filter(file=>file.toLowerCase().endsWith(".gpx")).length,0)});
